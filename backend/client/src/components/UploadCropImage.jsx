...
const UploadCropImage = () => {
  ...
  const [editedCropImage,  setEditedCropImage] = useState({
  ...
  const handleUpdateCropImage = async (e) => {
    e.preventDefault();
    try {
      const formData = new FormData();
      formData.append('image', editedCropImage.image);
      
      const response = await fetch(`http://localhost:4000/api/v1/crop/upload_image/${id}`, {
        method: 'PUT',
        headers: {
          'x-auth-token': token,
        },
        ...
        <div className="profile-wrapper-area py-3">
          <div className="card user-data-card">
            <div className="card-body">
              <form onSubmit={handleUpdateCropImage} >
              <div className="mb-3">
