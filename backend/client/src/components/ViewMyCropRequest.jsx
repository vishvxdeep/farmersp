...
const ViewMyCropRequest = () => {
  ...
  const UploadReceipt = (id) => {
    navigate("/upload_receipt_image/" + id);
  }
  ////////////////////////////////////////////////
  ...
  const [requestData, setCropRequestData] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [filteredData, setFilteredData] = useState([]);

  useEffect(() => {
    const fetchCropRequestData = async () => {
      try {
        ...
        const buyeremail = decodeURIComponent(document.cookie.replace(/(?:(?:^|.*;\s*)buyeremail\s*=\s*([^;]*).*$)|^.*$/, '$1'));
         ...
         const filteredCropRequest = data.filter((request) => request.buyeremail === buyeremail);
         setCropRequestData(filteredCropRequest);
         setFilteredData(filteredCropRequest);
              ...
              <div className="card product-card" style={{marginBottom:10}}>
                <div className="card-body"    >
                     ...
                     <a className="product-title d-block"  >Name:  <b> {request.name} </b></a>
                       ...
                       <a className="product-title d-block"  >Bidding Details: {request.bidding_details}  </a>
  ...
  {request.receipt && (
    <a href={`http://localhost:4000/${request.receipt}`} target="_blank" rel="noopener noreferrer" className="product-title d-block">
      ...
      <img src={`http://localhost:4000/${request.receipt}`} alt="receipt" style={{ maxWidth: '100%', maxHeight: '100%', width: 'auto', height: 'auto' }} />
                ...
                <a className="btn btn-danger" onClick={() => UploadReceipt(request.id)}>Upload Receipt</a> 
...
export default ViewMyCropRequest