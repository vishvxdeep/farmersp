...
const UpdateRate = () => {
  ...
  useEffect(() => {
    const fetchRateDetails = async () => {
      try {
        const response = await fetch(`http://localhost:4000/api/v1/rate/${id}`);
        if (response.ok) {
        ...
        <div className="profile-wrapper-area py-3">
          <div className="card user-data-card">
            ...
            <div className="mb-3">
                      ...
                      <input className="form-control" name="crop_name" id="name" value={editedRate.crop_name} onChange={handleInputChange} type="text" />
                    </div>
                   
                    <div className="mb-3">
                      <div className="title mb-2"><span>Price</span></div>
                      <input className="form-control" name="price" id="price" value={editedRate.price} onChange={handleInputChange} type="text" />
                    </div>
                
                    <div className="mb-3">
                      <div className="title mb-2"><span>Address</span></div>
                      <input className="form-control" name="address" id="address" value={editedRate.address} onChange={handleInputChange} type="text" />
                     </div>
                 
                    <div className="mb-3">
                      <div className="title mb-2"><span>Mobile</span></div>
                      <input className="form-control" name="mobile" id="mobile" value={editedRate.mobile} onChange={handleInputChange} type="text" />
                    </div>
