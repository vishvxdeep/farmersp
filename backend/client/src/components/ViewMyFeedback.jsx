...
const ViewMyFeedback = () => {
    ...
    const fetchLostfoundData = async () => {
      try {
        ...
        const serviceemail = decodeURIComponent(document.cookie.replace(/(?:(?:^|.*;\s*)serviceemail\s*=\s*([^;]*).*$)|^.*$/, '$1'));
         ...
         const filteredLostfound = data.filter((feedback) => feedback.useremail === serviceemail);
         setLostfoundData(filteredLostfound);
         setFilteredData(filteredLostfound);
  ...
  const handleSearch = (term) => {
    setSearchTerm(term);
    const filtered = lostfoundData.filter((lostfound) =>
      Object.values(lostfound).some((field) =>
